...
async function workWithPort(port: MessagePort) {
	port.onmessage = (event) => {
		...
		port.postMessage(
			...
			// This next line is what we'd like to uncomment in the following demo (324), but cannot:
			// [state], // """Value at index 0 does not have a transferable type""" great, thanks, wow.
		);
	};
}
